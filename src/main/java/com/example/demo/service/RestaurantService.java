...
public class RestaurantService {
	...
	public RestaurantModel changeStatus(@PathVariable int id,@PathVariable RestaurantModel resm) {
		System.out.println("hiiii i am in change  status method...."+id);
		String st="deactive";
		String st1="active";
			System.out.println(st+"   "+st1);
		    System.out.println("restaurant object......"+resm);
		    Optional<RestaurantModel> restdata=restaurantRepository.findById(id);
		    System.out.println("Optional data...."+restdata);
		    RestaurantModel rm=restdata.get();
		    System.out.println("model...."+rm);
		    if(restdata.isPresent()) {
				System.out.println(resm.getStatus());
				if(resm.getStatus().equals("active")) {
					rm.setStatus(st);
					System.out.println("after change rm..."+rm);
				}
				else {
					rm.setStatus(st1);
					System.out.println("after change in else..."+rm);
				}
		    }
